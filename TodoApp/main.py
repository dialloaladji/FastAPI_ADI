...
# Import Models module to register models with SQLAlchemy Base
import Models
...
async def get_todo_by_id(db: db_dependency, todo_id: int = Path(gt=0, description="The ID of the todo to retrieve")):
    """
...
async def update_todo(db: db_dependency, todo_id: Annotated[int, Path(gt=0, description="The ID of the todo to update")], todo_request: TodoRequest):
    """
...
async def delete_todo(db: db_dependency, todo_id: Annotated[int, Path(gt=0, description="The ID of the todo to delete")]):
    """
